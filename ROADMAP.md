...
Phase 2  Deployable MVP           ████████░░  11/13  <- current focus
...
**Next three, in order:** L20 (member management UI, whose endpoints have existed since
Phase 1) → L50 (the `DateTimeOffset` storage fix, which L16 worked around rather than solved
and which L51 and any future date filter both wait on) → L14. L22's remaining coverage and
L23's README are the cleanup tail.
